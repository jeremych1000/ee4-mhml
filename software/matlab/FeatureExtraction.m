...
%     feature_vec(index,10)=skewness(cell2mat(slice(:,2)));
%     feature_vec(index,11)=skewness(cell2mat(slice(:,3)));
%     feature_vec(index,12)=skewness(cell2mat(slice(:,6)));
%     feature_vec(index,13)=spectralEnergyRatio(cell2mat(slice(:,2)));
%     feature_vec(index,14)=spectralEnergyRatio(cell2mat(slice(:,3)));
    feature_vec(index,10)=kurtosis(cell2mat(slice(:,2)));
    feature_vec(index,11)=kurtosis(cell2mat(slice(:,3)));
    feature_vec(index,12) = label;
end
    featureVec = [featureVec;feature_vec];
%feature_vec=[mean_hr,std_hr,mean_rr,std_rr,mean_gsr,std_gsr,mean_temp,std_temp,mean_acc,skew_hr,skew_rr,skew_temp]
end